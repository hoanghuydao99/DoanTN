...
public class SanphamSaleAdapter extends RecyclerView.Adapter<SanphamSaleAdapter.ItemHoler> {
    Context context;
    ArrayList<Sanpham> arrayList;

    public SanphamSaleAdapter(Context context, ArrayList<Sanpham> arrayList) {
    ...
    public ItemHoler onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.customsale, null);
        ItemHoler itemHoler = new ItemHoler(v);
        return itemHoler;
    }

    @Override
    public void onBindViewHolder(@NonNull SanphamSaleAdapter.ItemHoler holder, int position) {
        Sanpham sanpham = arrayList.get(position);
        holder.txttensp.setText(sanpham.getTensp());
        int gia = Integer.parseInt(String.valueOf(sanpham.getGiasp()));
        double giamoi = gia*0.9;
        DecimalFormat decimalFormat = new DecimalFormat("###,###,###");
        holder.txtgiasp.setText(decimalFormat.format(giamoi)  + " VNĐ");
